...
(function(module, _) {
    ...
    function gameLogicFactory() {
        ...
        function checkPosition(figure, x, y) {
            var distance = 1;
            var posibleMoves = [];
            var priority = 1;
            //простой ход, без атаки
            while (board[distance * y - figure.y * (distance - 1)][distance * x - figure.x * (distance - 1)].value == '' && !board[distance * y - figure.y * (distance - 1)][distance * x - figure.x * (distance - 1)].unreacheble && (!figure.maxDistance || distance <= figure.maxDistance)) {
                posibleMoves.push({
                    x: distance * x - figure.x * (distance - 1),
                    y: distance * y - figure.y * (distance - 1),
                    priority: priority
                });
                distance++;
            }
            //если уперлись в свою шашку, или конец поля то считаем что поиски окончены и возвращаем возможные ходы
            if (board[distance * y - figure.y * (distance - 1)][distance * x - figure.x * (distance - 1)].owner == figure.owner ||
                board[distance * y - figure.y * (distance - 1)][distance * x - figure.x * (distance - 1)].unreacheble)
                return posibleMoves;
            //если уперлись в чужую шашку, надо проверить можно ли её срубить, 
            //для этого за ней должно быть свободное поле
            if (board[distance * y - figure.y * (distance - 1)][distance * x - figure.x * (distance - 1)].value.owner != figure.owner) {
                var closeX = distance * x - figure.x * (distance - 1);
                var closeY = distance * y - figure.y * (distance - 1);
                distance++;
                priority++;
                while (board[distance * y - figure.y * (distance - 1)][distance * x - figure.x * (distance - 1)].value == '' && !board[distance * y - figure.y * (distance - 1)][distance * x - figure.x * (distance - 1)].unreacheble && (!figure.maxDistance || distance - 1 <= figure.maxDistance)) {
                    //ок, поле свободно, значит приоритет такого хода повышается
                    if (canAttackAdditionalVictim(distance * x - figure.x * (distance - 1), distance * y - figure.y * (distance - 1), closeX, closeY, figure.owner) && !figure.maxDistance)
        ...
        function findVictim(myx, myy, x, y, myowner, closeX, closeY) {
            ...
            if (board[distance * y - myy * (distance - 1)][distance * x - myx * (distance - 1)].value.owner != myowner && !board[(distance + 1) * y - myy * distance][(distance + 1) * x - myx * distance].unreacheble && board[(distance + 1) * y - myy * distance][(distance + 1) * x - myx * distance].value == '')
                return true;
        }
        ...
        function possibleMove(draught) {
            ...
            if (figure.mainDirrection > 0) {
                values = values.concat(values, checkPosition(figure, figure.x + 1, figure.y + 1));
                values = values.concat(values, checkPosition(figure, figure.x - 1, figure.y + 1));
                priority = checkPosition(figure, figure.x - 1, figure.y - 1);
                if (draught.type == 2 || (priority.length > 0 && priority[0].priority == 2)) values = values.concat(values, priority);
                priority = checkPosition(figure, figure.x + 1, figure.y - 1);
                if (draught.type == 2 || (priority.length > 0 && priority[0].priority == 2)) values = values.concat(values, priority);
            }
            if (figure.mainDirrection < 0) {
